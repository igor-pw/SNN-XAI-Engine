...
public class MnistLayerwiseGradientFeatureAttributionTest
{
    ...
    public void shouldPerformLayerwiseRelevancePropagation() {
        ...
        for(int i = 0; i < predictSize; i++) {
            float [] target = new float[neuralNetwork.getLayer(neuralNetwork.getLayer().length - 1).getOutputSize()];
            target[expected[i]] = 1.0f;
            float [] featureAttribution = explainer.performLayerwiseGradientFeatureAttribution(target, normalizedInput[i],  neuralNetwork);
